...
TEST_F(IntegrationBasicAlgo, InitialLiquidity)
{
    ...
    trader2->modify_holdings(Ticker::ETH, 1000); // NOLINT
    // trader2->add_order({Ticker::ETH, sell, 100.0});
    trader2->add_order(limit_order{Ticker::ETH, sell, 100, 100.0});

    TestMatchingCycle cycle{traders};

    cycle.wait_for_order(limit_order{Ticker::ETH, buy, 100, 10.0});
}
...
TEST_F(IntegrationBasicAlgo, RemoveIOCOrder)
{
    ...
    auto trader2 = traders.add_trader<TestTrader>(0);
    trader2->modify_holdings(Ticker::ETH, 1000); // NOLINT
    trader2->add_order({Ticker::ETH, sell, 100, 100.0});
...
TEST_F(IntegrationBasicAlgo, MarketOrderBuy)
{
    ...
    auto trader2 = traders.add_trader<TestTrader>(0);
    trader2->modify_holdings(Ticker::ETH, 1000);
    trader2->add_order({Ticker::ETH, sell, 100, 100.0});
...
TEST_F(IntegrationBasicAlgo, MarketOrderSell)
{
    ...
    auto trader2 = traders.add_trader<TestTrader>(0);
    trader1.modify_holdings(Ticker::ETH, 1000);
...
TEST_F(IntegrationBasicAlgo, ManyUpdates)
{
    ...
    trader2->modify_holdings(Ticker::ETH, 100000); // NOLINT
...
TEST_F(IntegrationBasicAlgo, OnTradeUpdate)
{
    ...
    auto trader2 = traders.add_trader<TestTrader>(0);
    trader2->modify_holdings(Ticker::ETH, 10000); // NOLINT
