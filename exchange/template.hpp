...
class Strategy {
public:
    explicit Strategy(MarketOrderFunc);

    ~Strategy();

    /**
     * Called whenever two orders match. Could be oen of your orders, or two other
     * people's orders.
     *
     ...
     */
    void on_trade_update(std::string, std::string, double, double);
    /**
     ...
     */
    void on_orderbook_update(std::string, std::string, double, double);
    /**
     ...
     */
    void on_account_update(std::string, std::string, double, double, double);
...
private:
     ...
     */
    MarketOrderFunc place_market_order;
};
}
